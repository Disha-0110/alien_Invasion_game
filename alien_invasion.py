...
class AlienInvasion:
    ...
    def _reset_level(self):
        #Clear bullets and fleet, recreate aliens, re-center the ship
    ...
    #handles events like key press, key releases, and quitting
    ...
    #handles movements, shooting, quitting
    def _check_keydown_events(self, event):
        if event.key ==pygame.K_UP:
            self.ship.move_up()
        elif event.key ==pygame.K_DOWN:
            self.ship.move_down()
            if self.ship.rect.bottom >= self.settings.screen_h:
                self._reset_level()
